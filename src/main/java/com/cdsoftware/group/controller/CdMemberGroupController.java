...
public class CdMemberGroupController extends BaseController {
	...
	public void exportXls(CdMemberGroupEntity cdMemberGroup,HttpServletRequest request,HttpServletResponse response
			, DataGrid dataGrid) {
		...
		try {
			...
			List<CdMemberGroupEntity> cdMemberGroups = this.cdMemberGroupService.getListByCriteriaQuery(cq,false);
			workbook = ExcelExportUtil.exportExcel(new ExcelTitle("转发记录列表", "导出人:"+ResourceUtil.getSessionUserName().getRealName(),
					"导出信息"), CdMemberGroupEntity.class, cdMemberGroups);
	...
	public void exportXlsByT(CdMemberGroupEntity cdMemberGroup,HttpServletRequest request,HttpServletResponse response
			, DataGrid dataGrid) {
		response.setContentType("application/vnd.ms-excel");
		String codedFileName = null;
		OutputStream fOut = null;
		try {
			...
			HSSFWorkbook workbook = null;
			workbook = ExcelExportUtil.exportExcel(new ExcelTitle("转发记录列表", "导出人:"+ResourceUtil.getSessionUserName().getRealName(),
					"导出信息"), CdMemberGroupEntity.class, null);
			fOut = response.getOutputStream();
			workbook.write(fOut);
		} catch (Exception e) {
		} finally {
	...
	public AjaxJson importExcel(HttpServletRequest request, HttpServletResponse response) {
		...
		for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
			...
			try {
				List<CdMemberGroupEntity> listCdMemberGroupEntitys = 
					(List<CdMemberGroupEntity>)ExcelImportUtil.importExcelByIs(file.getInputStream(),CdMemberGroupEntity.class,params);
				for (CdMemberGroupEntity cdMemberGroup : listCdMemberGroupEntitys) {
					cdMemberGroupService.save(cdMemberGroup);
				}
				j.setMsg("文件导入成功！");
			} catch (Exception e) {
